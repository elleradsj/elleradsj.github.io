...
function addToDex(poke) {
	var dexObject = {};
	if (SETDEX_SWSH[poke.name] == undefined) SETDEX_SWSH[poke.name] = {};
...
function updateDex(customsets) {
	for (var pokemon in customsets) {
		for (var moveset in customsets[pokemon]) {
			if (!SETDEX_SWSH[pokemon]) SETDEX_SWSH[pokemon] = {};
			SETDEX_SWSH[pokemon][moveset] = customsets[pokemon][moveset];
